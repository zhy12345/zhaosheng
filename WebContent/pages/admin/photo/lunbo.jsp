...
<body>
		...
		<div class="row">
			...
			<div class=".table-responsive">
				<table class="table table-responsive table-striped table-bordered table-hover" style="cursor: pointer;" id="goodTable">
				 ...
				 <c:forEach items="${lunbolist}" var="list">
						<tr>
							<td><input type="checkbox" value="${list.id}" /></td> 
							<td>${list.id}</td>
							<td>${list.photodetail}</td>
							<td id="photo">
	...
	<div class="row" id="updateLunbo" style="display: none; margin: 0; padding: 20px;">
		<form action="admin/updatelunbo.shtml" class="form-horizontal" id="modifyLunbo" method="post" >
			<input type="hidden" id="id" name="id">
			
			<div class="form-group">
				<label class="col-sm-3 control-label">照片描述：</label>
				<div class="col-sm-9">
					<input type="text" id="photodetail" name="photodetail" class="form-control">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" id="orders" name="orders" class="form-control">
				</div>
			</div>
	...
	<div class="row" id="addLunboPhoto" style="display: none; margin: 0; padding: 20px;">
		<form action="admin/sss.shtml" class="form-horizontal" id="addPhoto" method="post" enctype="multipart/form-data">
			<input type="hidden" name="posution" value="主轮播">
			<div class="form-group">
			...
			<div class="form-group">
				<label class="col-sm-3 control-label" >照片描述：</label>
				<div class="col-sm-9">
					<input type="text" name="photodetail" class="form-control" value="测试专用">
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("#delete").click(
							function() {
								...
								var del = parent.layer.confirm("确定删除此照片么？删除后将不会复原！",function() {
													...
													location.href = "admin/dellunbophoto.shtml?delids="+ delIds;
			...
			$("#update").click(function() {
				...
				var photodetail =allchecked.parents("tr").find("td:eq(2)").text();
				var orders = allchecked.parents("tr").find("td:eq(4)").text();
				layer.open({
					type : 1,
					title : [ '修改轮播图片', 'font-size:14px;' ],
					skin : 'layui-layer-rim', //加上边框
					area : [ '600px', '300px' ], //宽高
					content : $('#updateLunbo'),
					success : function(layero, index) {
						$("#updateLunbo input[name=id]").val(updateId);
						$("#updateLunbo input[name=photodetail]").val(photodetail);
						$("#updateLunbo input[name=orders]").val(orders);
					},
				 btn:['修改'],
