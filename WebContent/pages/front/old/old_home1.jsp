...
<head>
    ...
    <!-- <link rel="alternate" href="css/vedio.css"> -->
    <style>
        body{
            max-width: 1500px;
            min-height: 1000px;
...
<body>
    ...
    <div id="body" style="margin-top: 30px;">
        ...
        <div class="nav_body">
            <div class="nav_body_1">
               <a href="<%=basePath %>/getallnews.shtml"><p class="p1">最新动态</p></a>
            </div>
            <div class="nav_body_1 nav_body_2">
             <a href="<%=basePath %>/pages/front/video1.jsp" ><p class="p1"> 青春风采</p></a>
            </div>
            <div class="nav_body_1 nav_body_3">
                <a href="<%=basePath %>/pages/front/check.jsp" ><p class="p1">录取查询</p></a>
            </div>
            <div class="nav_body_1 nav_body_4 ">
                 <a href="javaScript:void(0);" ><p id="zhaoShengPlan"class="p1"> 招生计划</p></a>
            </div>
        </div>
        <script>
        ...
        <div class="body">
            <div class="body_left">
                ...
                <div class="article">
                    <h2 class="video_text">
                        <a href="news.html"  name="article"><span>最新动态 </a><a class="more" href="<%=basePath %>/getallnews.shtml">查看更多...</a></span>
                    </h2>
                    <div class="xsxx-con fl">
                        <ul class="new_ul">
                        <c:forEach items="${fournews }" var="news">
                            <li class="new_li">
                                <div class="date fl" style="width:55px;">
                                <c:choose >
                                ...
                                <div class="txt fl" style="margin-top: 6px;">
                                    ...
                                    <i><p style="font-size:16px; width:400px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;">内容简介：${news.content}</p></i>
                ...
                <div class="video">
                    ...
                    <a class="more1" href="<%=basePath %>/pages/front/video1.jsp">查看更多...${list.videopath }</a>
                    <c:forEach  items="${videoList}" var="list"  varStatus="status">
                    	<div class="video_div" videopath = "${list.videopath }" style="margin-top: 10px;" ><a href="javaScript:;"><img src="${list.photopath}" class="video_img"></a></div>
                    </c:forEach>
                </div>
            ...
            <div class="body_right">
                ...
                <div class="theform ">
                    ...
                    <div class="search-form margin-top-md" id="sty_login" style="margin-bottom:5px;" >
                            ...
                            <form role="form"  action="checkluqu.shtml" method="post" role="form" >
		...
		<div class="row" id="planDiv" style="display:none ; margin: 0; padding: 20px;">
			<form action="<%=basePath%>/findPlanInfo.shtml" class="form-horizontal" id="findPlanInfo" method="post">
				<div class="form-group">
					<label class="col-sm-3 control-label">专业：</label>
					<div class="col-sm-9">
						<select id="major" name="major" class="form-control">
						<c:forEach items="${majorList}" var="list">
							<option value="${list.major }">${list.major }</option>
						</c:forEach>
						</select>
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-3 control-label">科类：</label>
					<div class="col-sm-9">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<select id="shengfen" name="province" class="form-control">
						</select>
					</div>
				</div>
			</form>
	...
	<script type="text/javascript" src="<%=basePath %>/assets1/lib/JQuery/jquery-3.3.1.min.js"></script>
	<script type="text/javascript" src="<%=basePath %>/assets1/lib/common/bootstrap/js/bootstrap.min.js"></script>
	<script type="text/javascript" src="<%=basePath %>/assets1/lib/layer/layer.js"></script>
	
	<script type="text/javascript">
		$("#zhaoShengPlan").click(function(){
			layer.open({
			    type: 1,
			    title: false,
			    shadeClose: true,
			    area: ['600px', '350px'],
			    title : [ '招生计划查询', 'font-size:14px;' ],
		...
		$("#major").click(function() {
			$("#families>option").remove();
			$("#shengfen>option").remove();
			var major = $("#major").val();
			$.ajax({
				url:"<%=basePath%>/getFamiliesList.shtml",
				type:"POST",
				dataType:"json",
				data:{major:major},
				success: function(result) {
					var count ="";
					for(var i in result){
						count += "<option value='"+ result[i].families+"'>"+result[i].families+"</option>";
					}
		...
		$("#families").click(function() {
			...
			$.ajax({
				url:"<%=basePath%>/getProviceList.shtml",
				type:"POST",
				dataType:"json",
				data:{major:major,families:families},
				success: function(result) {
					var count ="";
					for(var i in result){
						count += "<option value='"+ result[i].provice+"'>"+result[i].provice+"</option>";
					}
					$("#shengfen").append(count);
				}
			});
		});
...
<div class="actGotop">
    ...
    <!-- <img src="image/cloud.gif" alt="" class="img2"> -->
</div>
<div id="weixin">
