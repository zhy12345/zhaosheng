...
<body>
    <div class="top">
     ...
     <div style="margin-top: 70px;"></div>
        <div class="span_nav">
             <span >当前位置：<a href="home.jsp">招生信息网</a>&gt;&gt;&gt;<a href="major.jsp">专业介绍</a></span>
        </div>
    <div id="body">
            <div class="body_left">
                <h3>本科专业</h3>
                <ul class="undergraduate_course" id="adminTbody">
                    <p ><a href="bkmajor.shtml">查看更多+</a></p>
                    <c:forEach items="${bklist }" end="3" var="bk">
                    <a href="majordetail.shtml?id=${bk.id}&majortype=${bk.majortype}"><li> ${bk.name} <span>${bk.addtime } </span></li></a>
                    </c:forEach>
                </ul>
                 <h3>专科专业</h3>
                <ul class="college" id="adminTbody">
                    <p ><a href="zkmajor.shtml">查看更多+</a></p>
                     <c:forEach items="${zklist }" end="3" var="zk">
                   		 <a href="majordetail.shtml?id=${zk.id}&majortype=${zk.majortype}"><li>  ${zk.name } <span>${zk.addtime } </span></li></a>
                    </c:forEach>
                </ul>
                 <h3>专升本专业</h3>
                <ul class="to_college" id="adminTbody">
                    <p ><a href="zsbmajor.shtml">查看更多+</a></p>
                    <c:forEach items="${zsblist }" end="3" var="zsb">
                    	<a href="majordetail.shtml?id=${zsb.id}&majortype=${zsb.majortype}"><li> ${zsb.name }  <span>${zsb.addtime } </span></li></a>
                 	</c:forEach>
                </ul>
            </div>
